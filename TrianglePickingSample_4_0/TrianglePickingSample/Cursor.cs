...
namespace TrianglePicking
{
    ...
    /// </summary>
    public class Cursor : GraphicsDeviceControl
    {
        #region Constants
        ...
        public Cursor(GraphicsDevice graphics, ContentManager content)
        {
...
//#endif
        ...
        public void Update(DateTime dateTime)
        {
            ...
            if (currentState.DPad.Down == Microsoft.Xna.Framework.Input.ButtonState.Pressed)
            {
                delta.Y = 1;
            }
            if (currentState.DPad.Left == Microsoft.Xna.Framework.Input.ButtonState.Pressed)
            {
                delta.X = -1;
            }
            if (currentState.DPad.Right == Microsoft.Xna.Framework.Input.ButtonState.Pressed)
            {
                delta.X = 1;
            }
            ...
            //MouseState mouseState = Mouse.GetState();
            //position.X = PointToClient(MousePosition).X;
            //position.Y = PointToClient(MousePosition).Y;
        ...
        public Ray CalculateCursorRay(System.Drawing.Point mousePosition, Matrix projectionMatrix, Matrix viewMatrix)
        {
            // create 2 positions in screenspace using the cursor position. 0 is as
            // close as possible to the camera, 1 is as far away as possible.
            Vector3 nearSource = new Vector3(new Vector2(mousePosition.X, mousePosition.Y), 0f);
            Vector3 farSource = new Vector3(new Vector2(mousePosition.X, mousePosition.Y), 1f);
